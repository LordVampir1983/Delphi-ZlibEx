...
procedure GZDecompressString(var result: AnsiString; const s: RawByteString;
  var fileName, comment: AnsiString; var dateTime: TDateTime; ignore_trailer:boolean = false); overload;
...
procedure GZDecompressString(var result: AnsiString; const s: RawByteString; ignore_trailer:boolean = false);
  overload;
...
procedure GZDecompressString(var result: AnsiString; const s: RawByteString;
  var fileName, comment: AnsiString; var dateTime: TDateTime; ignore_trailer:boolean = false);
var
  index: Integer;
  crc  : Longint;
  size : Integer;
begin
  ...
  if not ignore_trailer then begin
...
procedure GZDecompressString(var result: UnicodeString;
  const s: RawByteString; var fileName, comment: AnsiString;
  var dateTime: TDateTime; ignore_trailer:boolean = false);
var
  index: Integer;
  crc  : Longint;
  size : Integer;
...
function GZDecompressStr(const s: RawByteString): AnsiString; overload;
begin
  GZDecompressString(result, s, false);
end;


function GZDecompressStr(const s: RawByteString; ignore_trailer:boolean): AnsiString; overload;
begin
  GZDecompressString(result, s, ignore_trailer);
end;


procedure GZDecompressString(var result: AnsiString; const s: RawByteString; ignore_trailer:boolean);
var
  fileName: AnsiString;
  comment : AnsiString;
  dateTime: TDateTime;
begin
  GZDecompressString(result, s, fileName, comment, dateTime, ignore_trailer);
end;
...
begin
  ...
  try
    ...
    begin
      tempFileName := IncludeTrailingBackslash(outFolder);
    ...
    begin
     hFile:=0;
     try
      hFile := FileOpen(outFileName, fmOpenWrite or fmShareDenyNone);
      if (hFile > 0) then
       FileSetDate(hFile, DateTimeToFileDate(dateTime));
     finally
