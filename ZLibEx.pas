...
function  ZDeflateInit(stream: PZStreamRec;
  level: TZCompressionLevel): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

function  ZDeflateInit2(stream: PZStreamRec;
  level: TZCompressionLevel; windowBits, memLevel: Integer;
  strategy: TZStrategy): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

function  ZDeflate(stream: PZStreamRec; flush: TZFlush): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

function  ZDeflateEnd(stream: PZStreamRec): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

function  ZDeflateReset(stream: PZStreamRec): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

{** zlib inflate routines ***********************************************************************}

function  ZInflateInit(stream: PZStreamRec): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

function  ZInflateInit2(stream: PZStreamRec;
  windowBits: Integer): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

function  ZInflate(stream: PZStreamRec; flush: TZFlush): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

function  ZInflateEnd(stream: PZStreamRec): Integer;
  {$ifdef Version2005Plus} inline; {$endif}

function  ZInflateReset(stream: PZStreamRec): Integer;
  {$ifdef Version2005Plus} inline; {$endif}
...
procedure ZDeflateEx(stream: PZStreamRec; param: Pointer;
  read: TZReadFunction; write: TZWriteFunction; flush: TZFlush);

procedure ZInflateEx(stream: PZStreamRec; param: Pointer;
  read: TZReadFunction; write: TZWriteFunction; flush: TZFlush);
...
begin
  ...
  if result = 0 then ZStreamInitCheck(stream);
end;
...
begin
  result := deflateInit2_(stream, ZLevels[level], Z_DEFLATED, windowBits,
    memLevel, ZStrategies[strategy], ZLIB_VERSION, SizeOf(TZStreamRecInt));
 if result = 0 then ZStreamInitCheck(stream);
end;
...
function ZInflateInit(stream: PZStreamRec): Integer;
begin
  result := inflateInit_(stream, ZLIB_VERSION, SizeOf(TZStreamRecInt));
  if result = 0 then ZStreamInitCheck(stream);
end;
...
procedure ZInternalCompressEx(zstream: PZStreamRec; const inBuffer: Pointer;
...
procedure ZInternalDecompressEx(zstream: PZStreamRec; const inBuffer: Pointer;
  inSize: Integer; out outBuffer: Pointer; out outSize: Integer;
  outEstimate: Integer);
var
  param: TZBufferParam;
...
procedure ZInternalCompress(zstream: PZStreamRec; const inBuffer: Pointer;
  inSize: Integer; out outBuffer: Pointer; out outSize: Integer);
const
  delta = 256;
var
...
procedure ZInternalDecompress(zstream: PZStreamRec; const inBuffer: Pointer;
  inSize: Integer; out outBuffer: Pointer; out outSize: Integer;
  outEstimate: Integer);
var
  zresult: Integer;
...
var
  zstream: TZStreamRecInt;
begin
  FillChar(zstream, SizeOf(TZStreamRecInt), 0);
...
procedure ZInternalCompressStreamEx(zstream: PZStreamRec; inStream,
  outStream: TStream);
var
  param: TZStreamParam;
begin
...
procedure ZInternalDecompressStreamEx(zstream: PZStreamRec; inStream,
  outStream: TStream);
var
  param: TZStreamParam;
begin
...
procedure ZInternalCompressStream(zstream: PZStreamRec; inStream,
  outStream: TStream);
const
  bufferSize = 32768;
var
...
begin
  ...
  begin
    ...
    repeat
      ...
      Assert((outsize>=0) and (outsize<=bufferSize), 'ZDeflate unrealistic value');
...
procedure ZInternalDecompressStream(zstream: PZStreamRec; inStream,
  outStream: TStream);
const
  bufferSize = 32768;
var
  ...
  try
    ...
    begin
      ...
      repeat
        ...
        Assert((outsize>=0) and (outsize<=bufferSize), 'ZInflate unrealistic value');
        if outsize>0 then
         outStream.Write(outBuffer, outSize);
      until (zresult = Z_STREAM_END) or (zstream.avail_out > 0);
...
begin
  ...
  FillChar(FZStream, SizeOf(TZStreamRecInt), 0);
